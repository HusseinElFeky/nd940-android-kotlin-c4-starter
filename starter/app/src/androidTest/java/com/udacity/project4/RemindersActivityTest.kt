...
class RemindersActivityTest : AutoCloseKoinTest() {
    ...
    fun addReminderAndNavigateBack() {
        ...
        // ReminderListFragment: Add new reminder.
        onView(withId(R.id.noDataTextView)).check(matches(withEffectiveVisibility(Visibility.VISIBLE)))
        onView(withId(R.id.addReminderFAB)).perform(click())

        // SaveReminderFragment: Select location.
        onView(withId(R.id.selectedLocation)).perform(click())

        // SelectLocationFragment: Set location.
        onView(withId(R.id.map)).perform(click())
        onView(withId(R.id.btn_save)).perform(click())

        // SaveReminderFragment: Enter reminder title and description, then save.
        onView(withId(R.id.reminderTitle)).perform(typeText("Title"))
        onView(withId(R.id.reminderDescription)).perform(typeText("Description"))
        closeSoftKeyboard()
        onView(withId(R.id.saveReminder)).perform(click())

        // ReminderListFragment: Verify that the reminder is displayed, and "No Data" text view is hidden.
